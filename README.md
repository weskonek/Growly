...
| `subscriptions`       | Parent subscription tier, status, and limits             |
...
| `daily_reports`       | Daily child growth insights generated by AI              |
...
7. **Subscription & Tier Gates** — Tier management (Free, Premium, School), feature gating (AI Tutor), child limit enforcement (client-side + DB triggers)
...
### Child App PIN Gate
```dart
Future<void> verifyPin(String pin, Function(String) onVerified) async {
  final data = await Supabase.instance.client
      ...
      .eq('is_active', true)
      .maybeSingle();
  if (data != null) onVerified(data['id'] as String);
}
```
...
Future<void> getRows() async {
  // Supabase query returns PostgrestList directly
